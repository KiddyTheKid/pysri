...
class Factura(models.Model):
    ...
    empresa_emisora_id = models.ForeignKey(
    ...
    autorizacion_numero = models.CharField(
        max_length=250, null=True, blank=True)
    autorizacion_fecha = models.DateTimeField(
    ...
    sri_fecha_respuesta = models.DateTimeField(
        default=None, null=True, blank=True)
    ...
    def save(self, **kwargs):
        if not self.secuencial:
            ultimo_reg = Factura.objects.filter(
                empresa_emisora_id=self.empresa_emisora_id).order_by('-secuencial').first()
            if ultimo_reg:
